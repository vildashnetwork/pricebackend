...
router.post("/", async (req, res) => {
    ...
    try {
                ...
                <div style="max-width: 650px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06); border: 1px solid #e2e8f0;">
                    ...
                    <div style="padding: 30px; background-color: #f1f5f9;">
                        <table style="width: 100%; border-collapse: collapse;">
                            <tr>
                                <td style="width: 48%; padding: 10px; background: #ffffff; border-radius: 8px; border: 1px solid #e2e8f0;">
                    ...
                    <div style="padding: 30px;">
                        ...
                        <table style="width: 100%; font-size: 14px; color: #475569; line-height: 2;">
                            <tr><td style="width: 120px; font-weight: 600;">Client:</td><td style="color: #0f172a;">${name}</td></tr>
                            <tr><td style="font-weight: 600;">Company:</td><td style="color: #0f172a;">${company || 'Personal Project'}</td></tr>
                            <tr><td style="font-weight: 600;">Email:</td><td>${email}</td></tr>
                            <tr><td style="font-weight: 600;">Phone:</td><td>${number}</td></tr>
                            <tr><td style="font-weight: 600;">Services:</td><td>${Array.isArray(services) ? services.join(', ') : services}</td></tr>
                        </table>
                        <h3 style="font-size: 14px; color: #0f172a; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px; margin-top: 30px;">Project Brief</h3>
                        <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 20px; border-radius: 8px; color: #334155;">${desc}</div>
                    </div>
                </div>
            </div>`
        });

        res.status(200).json("quote request sent successfully")
    }
    catch (err) {
        // --- IMPROVED LOGGING START ---
        console.error("CRITICAL ERROR in Quote Route:");
        
        if (err.response) {
            // This is the error coming back from Brevo's API
            console.error("BREVO API ERROR:", err.response.data);
            console.error("STATUS CODE:", err.response.status);
        } else if (err.request) {
            // The request was made but no response was received
            console.error("NO RESPONSE FROM BREVO:", err.message);
        } else {
            // Something happened in setting up the request or database save
        ...
        res.status(500).json({
            ...
            message: err.message,
            // Only send Brevo details to frontend during debugging
